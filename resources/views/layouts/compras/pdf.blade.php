...
<div id="details" class="clearfix">
        <div id="client">
          ...
          <h2 class="name">{{$proveedor->nombre}}</h2>
          <div class="to">RFC:</div>
          <h2 class="name">{{$proveedor->rfc}}</h2>
          <div class="to">Dirección</div>
          <div class="address">{{$proveedor->calle}} #{{$proveedor->numero}} {{$proveedor->colonia}} {{$proveedor->codigo_postal}}</div>
          <div class="address">{{$proveedor->municipio}} {{$proveedor->estado}} {{$proveedor->pais}}</div>
          <div class="to">Correo electrónico</div>
          <div class="email"><a href="mailto:lealheda@gmail.com">{{$proveedor->correo_electronico}}</a></div>
        </div>
        <div id="invoice">
          <h1>Compra</h1>
          <div class="date">Fecha de operación: {{$compra->created_at}}</div>
        </div>
      </div>
